...
sm_X_module = SoftMax(_input = num_classes).forward(X)
...
sm_X_module_log = SoftMax(_input = num_classes, log=True).forward(X)
...
sm_X_module = SoftMax(_input = num_classes).forward(logits)
...
sm_X_module_log = np.round(SoftMax(_input = num_classes, log=True).forward(logits),4)
...
model = Sequential(Linear(28 * 28, 100), Sigmoid(), Linear(100, 10), SoftMax(10))
#.... 
